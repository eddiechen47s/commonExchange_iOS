...
class AllRecordOrderViewModel {
    ...
    func getMyTradeListByStatus(completion: @escaping () -> Void) {
        ...
        APIManager.shared.handleFetchAPI(apiURL: APIPath.getMyTradeListByStatus.value, param: APIParam.allOrderRecord.value+"&token=\(token)", completion: { data, response, error in
            if let err = error {
                //發生錯誤
